...
export function CrmPage({tenantId}){
  ...
  const load=async()=>{
    if(!tenantId)return;
    try{setError("");const [profileData,dashboardData,contactsData,activitiesData,paymentsData]=await Promise.all([crmApi.profile(tenantId),crmApi.dashboard(tenantId),crmApi.contacts(tenantId),crmApi.activities(tenantId),crmApi.payments(tenantId)]);setProfile(profileData);setDashboard(dashboardData);setContacts(contactsData);setActivities(activitiesData);setPayments(paymentsData);}catch(error){setError(error.message||"No se pudo cargar el CRM");}
  };
