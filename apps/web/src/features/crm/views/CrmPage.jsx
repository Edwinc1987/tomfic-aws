...
export function CrmPage({tenantId}){
  ...
  const load=async()=>{
    if(!tenantId)return;
    try{setError("");const [contactsData,activitiesData]=await Promise.all([crmApi.contacts(tenantId),crmApi.activities(tenantId)]);setContacts(contactsData);setActivities(activitiesData);}catch(error){setError(error.message||"No se pudo cargar el CRM");}
  };
  ...
  return <section aria-label="CRM" className="space-y-4">
    <header><p className="text-xs font-semibold uppercase tracking-wide text-indigo-600">Panel del dueño</p><h1 className="text-2xl font-bold text-slate-900">CRM de clientes</h1><p className="text-sm text-slate-500">Contactos, actividades y seguimiento comercial</p></header>
