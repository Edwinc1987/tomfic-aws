...
export class PrismaCaptureRepository implements CaptureRepository{
  ...
  async createIdempotent(input:CaptureInput){
    return this.db.$transaction(async(tx)=>{
      ...
      const round=await tx.countRound.findFirst({where:{id:input.roundId,count:{tenantId:input.tenantId}}});
      ...
      return tx.capture.create({data:{operationId:input.operationId,productId:input.productId,roundId:input.roundId,quantity:input.quantity,condition:input.condition}});
    });
  }
}
