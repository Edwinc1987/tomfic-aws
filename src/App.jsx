...
const loadFromStorage = () => {
  try {
    ...
    if (data.productos?.length) G.productos = data.productos;
    if (data.usuarios?.length) G.usuarios = data.usuarios;
    if (data.ubicaciones) G.localizaciones = data.ubicaciones;
    if (data.ubicacionesTipos?.length) G.ubicacionesTipos = data.ubicacionesTipos;
    if (data.localizacionTipos?.length) G.localizacionTipos = data.localizacionTipos;
    if (data.inventario !== undefined) G.inventario = data.inventario;
    if (data.conteos) G.conteos = data.conteos;
    if (data.capturas) G.capturas = data.capturas;
    if (data.alertas) G.alertas = data.alertas;
    if (data.historial) G.historial = data.historial;
...
export default function TomficApp(){
  ...
  if(!usuario) return <Login lf={loginForm} setLf={setLoginForm} err={loginErr} onLogin={login} lastSaved={lastSaved||dataLoaded}/>;
  const p={usuario,setUsuario,G,rerender,showToast,lastSaved,limpiarDatos};
  return(
    <>
...
// ADMIN
// ─────────────────────────────────────────
function ModAdmin({usuario,setUsuario,G,rerender,showToast,lastSaved,limpiarDatos}){
  ...
  const props={usuario,setUsuario,G,rerender,showToast};
  return(
    <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"system-ui,sans-serif"}}>
      <div style={{background:"#0f172a",color:"white",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:54,position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          {lastSaved&&<span style={{fontSize:10,color:"#475569"}}>💾 {lastSaved}</span>}
...
function VBaseDatos({G,rerender,showToast}){
  ...
  const confirmarImport=()=>{
    if(!rawData)return;
    // Normalizar: crear copia del row con claves sin espacios
    const normalize=(row)=>{
      const n={};
    ...
    const mapped=rawData.map((rawRow,i)=>{
      ...
      return{
        id:"p"+i,
        ean:get("EAN13","CODIGOBARRAS","ean"),
        codigo:get("CODIGO","CODIGOINTERNO","codigo"),
        nombre:get("NOMBRE PRODUCTO","nombre","NOMBRE"),
        referencia:get("NOMBRE REFERENCIA","referencia"),
        categoria:get("NOMBRE CATEGORIA","categoria"),
        subcategoria:get("NOMBRE SUBCATEGORIA","subcategoria"),
        subgrupo:get("NOMBRE SUBGRUPO","subgrupo"),
        determinada:get("NOMBRE DETERMINADA","determinada"),
        localizacion:get("NOMBRE LOCALIZACION","localizacion"),
        ubicacion:get("NOMBRE UBICACIÓN","NOMBRE UBICACION","ubicacion"),
        observacion:get("OBSERVACION","observacion"),
        saldo:parseFloat(row.SALDO||row.CANTIDAD||row.saldo||0)||0,
    ...
    rerender();showToast(`✓ ${mapped.length} productos importados`);
  };
...
// ─────────────────────────────────────────
function ModCapturador({usuario,setUsuario,G,rerender,showToast}){
